...
export interface VaultNavResponse {
  ...
  positions?: { protocol: string; symbol: string; kind: string; amount: string; decimals: number; usdValue: number | null }[];
...
export async function getDeploymentUpstream(id: string, userId: string): Promise<DeploymentRecord> {
  ...
  return {
    ...
    sourceVerification: (r.sourceVerification as DeploymentRecord['sourceVerification']) ?? null,
