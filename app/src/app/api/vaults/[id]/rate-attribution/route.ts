...
import {
  ...
  getVaultRateAttributionUpstream,
  UpstreamError,
...
// Share price attribution for a vault the caller owns. Resolves the deployment
// (owner-scoped by the backend) to the boringVault address, then reads the
// public /rate-attribution endpoint.
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  const { userId } = await auth();
  if (!userId) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  }
  if (!rateLimit(`rate-attribution:${userId}`, 60, 60_000)) {
  ...
  try {
    ...
    const attribution = await getVaultRateAttributionUpstream(vault, userId);
    return NextResponse.json(attribution);
  } catch (err) {
    if (err instanceof UpstreamError) {
      return NextResponse.json({ error: err.message }, { status: err.status });
    }
    console.error('[api/vaults/:id/rate-attribution]', err);
